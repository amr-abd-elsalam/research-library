...
describe('AnswerGroundingChecker', () => {
  ...
  it('T-AGC01: returns score 1 and totalClaims 0 when disabled', () => {
    // Config default is enabled: false, no override set
    const result = checker.check('المنصة توفر مساعد بحثي ذكي', 'بعض المحتوى');
  ...
  it('T-AGC02: empty answer returns score 1 when enabled', () => {
    featureFlags.setOverride('GROUNDING', true);
    const result = checker.check('', 'بعض المحتوى');
    assert.strictEqual(result.score, 1);
    assert.strictEqual(result.totalClaims, 0);
    assert.strictEqual(result.groundedClaims, 0);
  });
  ...
  it('T-AGC03: null answer returns score 1 when enabled', () => {
    featureFlags.setOverride('GROUNDING', true);
    const result = checker.check(null, 'بعض المحتوى');
    assert.strictEqual(result.score, 1);
    assert.strictEqual(result.totalClaims, 0);
  });

  // T-AGC04: enabled — answer fully grounded in context → score close to 1.0
  it('T-AGC04: fully grounded answer returns high score', () => {
    featureFlags.setOverride('GROUNDING', true);
    ...
    const result = checker.check(answer, context);
  ...
  it('T-AGC05: completely ungrounded answer returns low score', () => {
    featureFlags.setOverride('GROUNDING', true);
  ...
  it('T-AGC06: partially grounded answer returns intermediate score', () => {
    featureFlags.setOverride('GROUNDING', true);
  ...
  it('T-AGC07: diacritics are removed for comparison', () => {
    featureFlags.setOverride('GROUNDING', true);
  ...
  it('T-AGC08: claims split on sentence boundaries', () => {
    featureFlags.setOverride('GROUNDING', true);
  ...
  it('T-AGC09: maxClaimsToCheck limits number of claims checked', () => {
    featureFlags.setOverride('GROUNDING', true);
  ...
  it('T-AGC10: single claim answer works correctly', () => {
    featureFlags.setOverride('GROUNDING', true);
